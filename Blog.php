...
class ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	...
	function get_fields() {
      ...
      /**
			 * JEM Filters the tags added to the post content.
			 *
			 * This filter is used to modify the tags that are added to the post content.
			 * It allows developers to add, remove, or modify the tags before they are
			 ...
			 */
			'filter_tags' => array(
				'label'            => esc_html__( 'Filter by Tags', 'et_builder' ),
				'type'             => 'multiple_checkboxes',
				'option_category'  => 'basic_option',
				'options'          => $this->get_available_tags(),
				'description'      => esc_html__( 'Select tags to filter the posts.', 'et_builder' ),
				'toggle_slug'      => 'main_content',
				'computed_affects' => array(
					'__posts',
				),
...
*/
	...
	static function get_blog_posts( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		$defaults = array(
			...
			'fullwidth'                     => '',
			'posts_number'                  => '',
      'filter_tags'                   => '', // JEM Add this to filter tags
      //'filter_tag'                    => '', // JEM Add this to filter tags
			'include_categories'            => '',
			'meta_date'                     => '',
			'show_thumbnail'                => '',
			'show_content'                  => '',
      ...
      // 			'tag' => ! empty( $args['filter_tag'] ) ? sanitize_text_field( $args['filter_tag'] ) : '', // JEM
			'tag' => 'AC60', // ! empty( $args['filter_tags'] ) ? implode( ',', array_map( 'sanitize_text_field', $args['filter_tags'] ) ) : '', // JEM
		);
		...
		// JEM
		if ( ! empty( $args['filter_tags'] ) ) {
			// $query_args['tag'] = sanitize_text_field( $args['filter_tags'] );
			$query_args['tag'] = 'AC60'; //implode( ',', array_map( 'sanitize_text_field', $args['filter_tags'] ) );
		}
    ...
    // JEM
		if ( ! empty( $args['filter_tags'] ) ) {
			// $args['tag'] = sanitize_text_field( $args['filter_tags'] );
			$args['tag'] = 'AC60'; //implode( ',', array_map( 'sanitize_text_field', $args['filter_tags'] ) );
		}
